...
export const initializeApp = function (data) {
  ...
  statusIcon.addEventListener("click", function () {
    ...
    myStatusContainer.addEventListener("click", function () {
      status.viewMyStatus(myStatus[0]);
    });
    const recentContainer = document.querySelectorAll(".recent-status");
    console.log(myStatusContainer);
    recentContainer.forEach((c, i) => {
      c.addEventListener("click", function () {
        console.log(c);
        const id = +c.getAttribute("dataset-id");
        console.log(id);
        status.viewMyStatus(statusData[id]);
      });
    });
  });
